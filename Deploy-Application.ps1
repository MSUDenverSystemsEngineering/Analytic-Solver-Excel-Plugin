...
Try {
	...
	[string]$appVendor = 'Frontline Solvers'
	[string]$appName = 'Analytic Solver Excel Plugin'
	[string]$appVersion = '19.0.1'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		#Extract SolverSetup64 from the downloaded exe by grabbing it out of temp
		Execute-Process -Path "$dirFiles\SolverSetup64.exe" -Parameters '/s /v/qn' -WindowStyle 'Hidden'
		...
		#Adds startup shortcut to register application which will add add-in for each user. Yeah it's hacky but it's the only way they support
		Copy-File -Path "$dirFiles\Register Solver Addin.lnk" -Destination "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp"
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$exitCode = Remove-MSIApplications -Name "Frontline Excel Solvers" -PassThru
		...
		Remove-File -Path "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\Register Solver Addin.lnk"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAWcrRY3SKGGkGf
# nuP1wStR3T+QUZZl/G3InP+ywxsLD6CCFFwwggQUMIIC/KADAgECAgsEAAAAAAEv
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQggyWuj52a
# 3dLn2pMlbrMyb3oknizDomHkWvXm0kDH4rAwDQYJKoZIhvcNAQEBBQAEggEAwCrb
# ZnVdlCNZM7a1SKLXVS8F5LVfSrGO4EkJbxgRY/O8CrJxgWroWndc/BOt6crdZMPK
# RVhHmLAyjq569CROix5RSG4MNpLWSzUwytiVS590WYvHvCj11QWboDlsW2D3jfCw
# ORjacQSNen7bkudP3qKJp0eFOXzcqAfSBmrhhlBdWNiE7pikd6G9cbkDS6PAYzd4
# I8ctARqJ3emDZ0WvxA2in01JbyK8HdKIFUbrUfnazmbv+xr8oKxT09QlrX2oRtak
# So1lfvwkFm29FjwzRApkOZdv6TT0JjZdehUgEkd38/o9B/J091W4kht6m6i6uEsx
# V13JrzBCFdMkP3z2+qGCAqIwggKeBgkqhkiG9w0BCQYxggKPMIICiwIBATBoMFIx
...
# CSqGSIb3DQEJBTEPFw0xOTA4MDEyMzQ1MDNaMCMGCSqGSIb3DQEJBDEWBBQA1Gdg
# uaq/My4B8VHO5I7rqXvDhTCBnQYLKoZIhvcNAQkQAgwxgY0wgYowgYcwgYQEFGO4
...
# ggEAUo85ix42VDoLkxJ9Ax5vGQWuZ3bLlu5PfYpuZ/YpbTDyrtvRfaPmdtsLW74b
# 4bBObZSwmTaIjyaR7v2ZlXni4EHDEV2ih9vYhBYH+/9faRkXGAdpQ2TMXMc1fFSL
# eqqm/BBQ6ZrfDyl5KbSQMh2dRvM45tmIjSeXcraLTEK65kfAYmm1Hk67hY4Yv7dY
# LT1KuyiQZk8MNKbj5PdwnEAi8MBHEfhlH1zBKfLc8nsTg7dc47O9n+h1wrRGqYcj
# a6BUqUclCUI7lisHlacvsJPsAcZmKnOU3lpZlwFaT1YIRbbcvos2on4ZddFaJfo+
# l0Wa3d1rpYT7Rcci9mEbDAN+9A==
